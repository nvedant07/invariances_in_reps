...
from dolly.training.consts import (
    DEFAULT_INPUT_MODEL,
    DEFAULT_SEED,
...
from data_utils import slice_dataset, DATASET_VERSIONS
...
def load_training_dataset(path_or_dataset: str, partial: str, cache_dir: str) -> Dataset:
    logger.info(f"Loading dataset from {path_or_dataset}")
    dataset = slice_dataset(
        load_dataset(path_or_dataset, 
                     cache_dir=cache_dir, 
                     revision=DATASET_VERSIONS[path_or_dataset.replace('/','--')])["train"],
        partial)
...
def preprocess_dataset(tokenizer: AutoTokenizer, max_length: int, seed=DEFAULT_SEED, *,
                       path_or_dataset: str = "databricks/databricks-dolly-15k", 
                       partial_dataset: str = None, cache_dir: str = None) -> Dataset:
    ...
    dataset = load_training_dataset(path_or_dataset, partial_dataset, cache_dir)
...
def train(
    *,
    ...
    processed_dataset = preprocess_dataset(
        ...
        path_or_dataset=finetuning_ds, partial_dataset=partial_dataset, 
    ...
    if partial_dataset is not None:
        local_output_dir += f'-{partial_dataset}'
    local_output_dir += f'-pdbs{per_device_train_batch_size}'
...
@click.option("--resume-path", type=str, help="Where to resume training from", default=None)
@click.option("--dbfs-output-dir", type=str, help="Sync data to this path on DBFS")
@click.option("--ds-cache-dir", type=str, help="Where to store HF dataset", default=None)
@click.option("--finetuning-ds", type=str, help="Dataset name in HF hub", default="databricks/databricks-dolly-15k")
@click.option("--partial-dataset", type=str, help="Whether to use only a subset of the instruction dataset", default=None)
@click.option("--wandb-project-name", type=str, help="Project name for wandb", default='huggingface')
@click.option("--wandb-run-name", type=str, help="(Optional) run name, useful when resuming", default=None)
...
@click.option("--gradient-accumulation-steps", type=int, default=1, help="Steps count for which to accumulate gradients.")
@click.option(
